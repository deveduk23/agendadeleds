...
## 9) Programa pronto (CLI)

Arquivo: `programa_prospeccao.py`

### O que faz
- Cadastra leads em CSV (`add`)
- Gera bloco manual de mensagens (`bloco`)
- Registra eventos (`registrar`)
- Mostra KPIs básicos (`kpi`)

### Exemplos
```bash
python3 programa_prospeccao.py add --nome "João" --empresa "Metal X" --segmento "metalurgica"
python3 programa_prospeccao.py bloco --limite 5
python3 programa_prospeccao.py registrar --nome "João" --empresa "Metal X" --evento enviado
python3 programa_prospeccao.py kpi
```
